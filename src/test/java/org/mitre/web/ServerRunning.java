...
public class ServerRunning implements MethodRule {
	...
	public Statement apply(final Statement base, FrameworkMethod method, Object target) {
		...
		try {
			client.getForEntity(new UriTemplate(getUrl("/openid-connect-server-webapp/login")).toString(), String.class);
			online = true;
		...
		return new Statement() {
			...
			public void evaluate() throws Throwable {
				try {
					postForStatus("/openid-connect-server-webapp/oauth/uncache_approvals", new LinkedMultiValueMap<String, String>());
					base.evaluate();
				} finally {
					postForStatus("/openid-connect-server-webapp/oauth/cache_approvals", new LinkedMultiValueMap<String, String>());
				}
